...
void initCCU61(void)
{
    ...
    // CCU61_T12PR.U = 24414 -1;                           // PM interrupt freq. = f_T12 / (T12PR + 1)
    CCU61_T12PR.U = 48828 - 1;
    CCU61_TCTR4.U |= 0x1 << T12STR_BIT_LSB_IDX;         // load T12PR from shadow register

    CCU61_T12.U = 0;                                    // clear T12 counter register

    // CCU60 T12 PM interrupt setting
    CCU61_INP.U &= ~(0x3 << INPT12_BIT_LSB_IDX);        // service request output SR0 selected
    CCU61_IEN.U |= 0x1 << ENT12PM_BIT_LSB_IDX;          // enable T12 PM interrupt

    // SRC setting for CCU61
    SRC_CCU6_CCU61_SR0.U &= ~(0xFF << SRPN_BIT_LSB_IDX);
    SRC_CCU6_CCU61_SR0.U |= 0x0B << SRPN_BIT_LSB_IDX;   // set priority 0x0B

    SRC_CCU6_CCU61_SR0.U &= ~(0x2 << TOS_BIT_LSB_IDX);  // CPU1 service T12 PM interrupt
    SRC_CCU6_CCU61_SR0.U |= (0x2 << TOS_BIT_LSB_IDX);  // CPU1 service T12 PM interrupt

    SRC_CCU6_CCU61_SR0.U |= 0x1 << SRE_BIT_LSB_IDX;     // SR0 enabled

    // CCU61 T12 counting start
    CCU61_TCTR4.U = 0x1 << T12RS_BIT_LSB_IDX;           // T12 start counting
}
